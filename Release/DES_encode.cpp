...
char* CDes::Inputkey(){
	...
	while (!bflag)
	{
		scanf_s("%s", ctempsrc,16);
		while (*(ctempsrc + ilenth) != '\0') ilenth++;
		if (ilenth< 16)
			printf("秘钥长度小于16位，请重新输入！\n");
		else{
			for (int i = 0; i < 16; i++)
			{
		...
		for (int j = 1; j < 16; j++)
		{
			...
			for (int k = 0; k < 64; k++)
			{
				...
				if (i == 29 && j*k == 7 * 62){
					...
					getchar();
					if ((cchoose[0] == 'y' || cchoose[0] == 'Y') && bleft == bright)
						printf("回答正确！\n");
					else if ((cchoose[0] == 'n' || cchoose[0] == 'N') && bleft != bright)
						printf("回答正确！\n");
					else
						printf("回答错误！\n");
				}
